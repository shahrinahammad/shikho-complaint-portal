...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // ⚠️ এখানে আমরা আসল এরর মেসেজ ধরার জন্য অ্যালার্ট এবং কনসোল লগ বসিয়েছি
    if (error) {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (error) alert('সমস্যা হয়েছে: ' + error.message)
    else alert('✅ আপনার ইমেইলে পাসওয়ার্ড রিসেট করার লিংক পাঠানো হয়েছে! ইনবক্স চেক করুন।')
    setLoading(false)
  }
      ...
      <div className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-md border border-gray-100">
        ...
        <form onSubmit={isResetMode ? handleResetPassword : handleLogin} className="space-y-5">
          ...
          <button type="submit" disabled={loading} className="w-full bg-[#CF278D] text-white font-black p-3 rounded-xl hover:bg-pink-700 transition-all shadow-md">
            {loading ? 'দয়া করে অপেক্ষা করুন...' : (isResetMode ? 'পাসওয়ার্ড রিসেট লিংক পাঠান' : 'লগইন করুন')}
          </button>
        </form>

        <div className="mt-6 text-center">
          <button onClick={() => setIsResetMode(!isResetMode)} className="text-sm font-bold text-indigo-600 hover:underline">
            {isResetMode ? 'লগইন পেজে ফিরে যান' : 'পাসওয়ার্ড ভুলে গেছেন?'}
          </button>
        </div>
      </div>
    </div>
